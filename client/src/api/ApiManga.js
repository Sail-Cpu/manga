...
export const get = {
  ...
  fetchMangas: async (collection, page, search, pageSize, date) => {
    const endpoint = `${BASE_LINK}/mangas?collection=${collection}&page_size=${pageSize}&page=${page}&search=${search}&date=${date}`;
    return await (
      await axios.get(endpoint)
    ).data;
  },
